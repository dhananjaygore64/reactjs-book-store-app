...
const Home = ({books, setBooks, showAdd}) => {
    ...
    const onEditBook = (idx, key, event) => {
      ...
      const onEditBtnClicked = (_id, event) => {
        event.preventDefault();
        setDisabledObj({ ...disabledObj, [_id]: disabledObj[_id] === undefined ? false : !disabledObj[_id] });
      }
    
      const onDeleteBtnClicked = (_id, event) => {
        event.preventDefault();
        fetch(`http://localhost:3001/book/${_id}`, {
          headers: {
    ...
    return (
        <div>
            <div className="container">
                ...
                <Table books={books} onEditBook={onEditBook} disabledObj={disabledObj} onEditBtnClicked={onEditBtnClicked} onDeleteBtnClicked={onDeleteBtnClicked} onSaveBtnClicked={onSaveBtnClicked} />
            </div>
